...
@Component({
  ...
  animations: [
    ...
    trigger('planScale', [
      state('zoom', style({ 'margin-top': '0', 'margin-left': '0', width: '100%', height: '600px' })),
      state('scale', style({ 'margin-top': '60px', 'margin-left': 'calc(calc(100% - 300px) / 2)', width: '300px', height: '440px' })),
      transition('* => zoom', [
        animate('0.5s', style({ width: '100%', height: '600px', 'margin-left': '0', 'margin-top': '0' }))
      ]),
      transition('zoom => scale', [
        animate('0.3s', style({ 'margin-top': '60px', height: '440px', 'margin-left': 'calc(calc(100% - 300px) / 2)', width: '300px' }))
      ])
    ]),
    trigger('show', [
      transition(':enter', [
